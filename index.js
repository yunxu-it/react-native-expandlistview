...
export class ExpandableListView extends React.Component<Props> {
  ...
  componentWillReceiveProps ({initialGroups = [], data = [], allOpen = null}, nextContext) {
    let array = new Array(data.length)
    let initItem
    if (allOpen === null) {
      initItem = array.fill(false).map((item, index) => {
        return initialGroups.indexOf(index) !== -1
      })
    } else {
      initItem = array.fill(allOpen)
    }
    this.setState({
      groupStatus: initItem
    })
  }
